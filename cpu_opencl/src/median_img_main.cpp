...
int main(int argc, char* argv[]) {
    if(argc != 6) {
        printf(
            "Usage:\nfir-img 'input filename' 'output filename' '#of runs "
            "scalar' '#of runs vector' '#of runs kernel'\nExiting.\n");
        return -1;
    } else {
        printf("Input file: %s\n", argv[1]);
    ...
    for(row = 0; row < imgHeight; row++) {
        for(col = 0; col < imgWidth; col++) {
            int pixel_dst =
                ((row + imgFOfssetH) * imgWidthF + (col + imgFOfssetW)) * 3;
            int pixel_src = (row * imgWidth + col) * 3;
            *(imgSrcExt + pixel_dst + 0) =
                (uint8_t) (*(imgData + pixel_src + 0));
            *(imgSrcExt + pixel_dst + 1) =
                (uint8_t) (*(imgData + pixel_src + 1));
            *(imgSrcExt + pixel_dst + 2) =
                (uint8_t) (*(imgData + pixel_src + 2));
        }
    }
    ...
    for(int32_t run = 0; run < number_of_runs_scalar; run++) {
        median2d_c_batcher(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
    }
    ts_end = get_ts_ns();
    elapsed = ts_end - ts_start;

    perf = 1000.0 * double(imgHeight * imgWidth) *
           double(number_of_runs_scalar) / elapsed;
    printf("CPU_BATCHER time: %f ms, MPixels/s: %f\n",
           (elapsed / 1000000.0),
           perf);
#endif
    ...
    ts_start = get_ts_ns();
    for(int32_t run = 0; run < number_of_runs_scalar; run++) {
        median2d_c_nthelement(
            imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
    }
    ts_end = get_ts_ns();
    elapsed = ts_end - ts_start;
    ...
    perf = 1000.0 * double(imgHeight * imgWidth) *
           double(number_of_runs_scalar) / elapsed;
    printf("CPU_NTHELEMENT time: %f ms, MPixels/s: %f\n",
           (elapsed / 1000000.0),
           perf);
#endif
    ...
    for(int32_t run = 0; run < number_of_runs_vector; run++) {
        median2d_neon_uload(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
    }
    ts_end = get_ts_ns();
    elapsed = ts_end - ts_start;

    perf = 1000.0 * double(imgHeight * imgWidth) *
           double(number_of_runs_vector) / elapsed;
    printf(
        "CPU NEON time: %f ms, MPixels/s: %f\n", (elapsed / 1000000.0), perf);
    #endif

    #if 1
    ts_start = get_ts_ns();
    for(int32_t run = 0; run < number_of_runs_vector; run++) {
        median2d_neon_sh(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
    }
    ts_end = get_ts_ns();
    elapsed = ts_end - ts_start;
    ...
    ts_start = get_ts_ns();
    for(int32_t run = 0; run < number_of_runs_vector; run++) {
        median2d_avx_uload(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
    }
    ts_end = get_ts_ns();
    elapsed = ts_end - ts_start;
...
#if 1
    // Write output image OCL
    for(row = 0; row < imgHeight; row++) {
        for(col = 0; col < imgWidth; col++) {
            int pixel_src = (row * imgWidth + col) * 3;
            int pixel_dst = (row * imgWidth + col) * 3;
            *(imgData + pixel_dst + 0) =
                (ILubyte) (*(imgResOCL + pixel_src + 0));
            *(imgData + pixel_dst + 1) =
                (ILubyte) (*(imgResOCL + pixel_src + 1));
            *(imgData + pixel_dst + 2) =
                (ILubyte) (*(imgResOCL + pixel_src + 2));
        }
    }
#endif
