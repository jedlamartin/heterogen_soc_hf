...
void batcher_sort_5x5(uint8_t* elements,
                      size_t first = 0,
...
void batcher_sort_5x5(uint8_t* w) {
    for(int i = 0; i < 3; i++) {
        ...
        compare_swap(w, 12 * 3 + i, 14 * 3 + i);
        compare_swap(w, 13 * 3 + i, 15 * 3 + i);
        compare_swap(w, 16 * 3 + i, 18 * 3 + i);
        compare_swap(w, 17 * 3 + i, 19 * 3 + i);
        compare_swap(w, 20 * 3 + i, 22 * 3 + i);
        ...
        // p = 4
        // k = 4
        compare_swap(w, 0 * 3 + i, 4 * 3 + i);
        compare_swap(w, 1 * 3 + i, 5 * 3 + i);
        compare_swap(w, 2 * 3 + i, 6 * 3 + i);
        compare_swap(w, 3 * 3 + i, 7 * 3 + i);
        compare_swap(w, 8 * 3 + i, 12 * 3 + i);
        compare_swap(w, 9 * 3 + i, 13 * 3 + i);
        compare_swap(w, 10 * 3 + i, 14 * 3 + i);
        compare_swap(w, 11 * 3 + i, 15 * 3 + i);
        compare_swap(w, 16 * 3 + i, 20 * 3 + i);
        compare_swap(w, 17 * 3 + i, 21 * 3 + i);
        compare_swap(w, 18 * 3 + i, 22 * 3 + i);
        compare_swap(w, 19 * 3 + i, 23 * 3 + i);
        // k = 2
        ...
        // p = 8
        // k = 8
        compare_swap(w, 0 * 3 + i, 8 * 3 + i);
        compare_swap(w, 1 * 3 + i, 9 * 3 + i);
        compare_swap(w, 2 * 3 + i, 10 * 3 + i);
        compare_swap(w, 3 * 3 + i, 11 * 3 + i);
        compare_swap(w, 4 * 3 + i, 12 * 3 + i);
        compare_swap(w, 5 * 3 + i, 13 * 3 + i);
        compare_swap(w, 6 * 3 + i, 14 * 3 + i);
        compare_swap(w, 7 * 3 + i, 15 * 3 + i);
        compare_swap(w, 16 * 3 + i, 24 * 3 + i);
        // k = 4
        compare_swap(w, 4 * 3 + i, 8 * 3 + i);
        compare_swap(w, 5 * 3 + i, 9 * 3 + i);
        compare_swap(w, 6 * 3 + i, 10 * 3 + i);
        compare_swap(w, 7 * 3 + i, 11 * 3 + i);
        compare_swap(w, 20 * 3 + i, 24 * 3 + i);
        // k = 2
        compare_swap(w, 2 * 3 + i, 4 * 3 + i);
        compare_swap(w, 3 * 3 + i, 5 * 3 + i);
        ...
        // p = 16
        // k = 16
        compare_swap(w, 0 * 3 + i, 16 * 3 + i);
        compare_swap(w, 1 * 3 + i, 17 * 3 + i);
        compare_swap(w, 2 * 3 + i, 18 * 3 + i);
        compare_swap(w, 3 * 3 + i, 19 * 3 + i);
        compare_swap(w, 4 * 3 + i, 20 * 3 + i);
        compare_swap(w, 5 * 3 + i, 21 * 3 + i);
        compare_swap(w, 6 * 3 + i, 22 * 3 + i);
        compare_swap(w, 7 * 3 + i, 23 * 3 + i);
        compare_swap(w, 8 * 3 + i, 24 * 3 + i);
        // k = 8
        compare_swap(w, 8 * 3 + i, 16 * 3 + i);
        compare_swap(w, 9 * 3 + i, 17 * 3 + i);
        compare_swap(w, 10 * 3 + i, 18 * 3 + i);
        compare_swap(w, 11 * 3 + i, 19 * 3 + i);
        compare_swap(w, 12 * 3 + i, 20 * 3 + i);
        compare_swap(w, 13 * 3 + i, 21 * 3 + i);
        compare_swap(w, 14 * 3 + i, 22 * 3 + i);
        compare_swap(w, 15 * 3 + i, 23 * 3 + i);
        // k = 4
        compare_swap(w, 4 * 3 + i, 8 * 3 + i);
        compare_swap(w, 5 * 3 + i, 9 * 3 + i);
        compare_swap(w, 6 * 3 + i, 10 * 3 + i);
        compare_swap(w, 7 * 3 + i, 11 * 3 + i);
        compare_swap(w, 12 * 3 + i, 16 * 3 + i);
        compare_swap(w, 13 * 3 + i, 17 * 3 + i);
        compare_swap(w, 14 * 3 + i, 18 * 3 + i);
        compare_swap(w, 15 * 3 + i, 19 * 3 + i);
        compare_swap(w, 20 * 3 + i, 24 * 3 + i);
        // k = 2
        compare_swap(w, 2 * 3 + i, 4 * 3 + i);
        compare_swap(w, 3 * 3 + i, 5 * 3 + i);
...
void median2d_c_nthelement(int32_t imgHeight,
                           int32_t imgWidth,
                           int32_t imgWidthF,
                           uint8_t* imgSrcExt,
                           uint8_t* imgDst) {
#pragma omp parallel for
    for(int row = 0; row < imgHeight; ++row) {
        uint8_t rbuffer[M_HEIGHT * M_WIDTH];
        uint8_t gbuffer[M_HEIGHT * M_WIDTH];
        uint8_t bbuffer[M_HEIGHT * M_WIDTH];

        for(int col = 0; col < imgWidth; ++col) {
            for(size_t frow = 0; frow < M_HEIGHT; ++frow) {
                for(size_t fcol = 0; fcol < M_WIDTH; ++fcol) {
                    // r
                    rbuffer[frow * M_WIDTH + fcol] =
                        imgSrcExt[((row + frow) * imgWidthF + col + fcol) * 3];

                    // g
                    gbuffer[frow * M_WIDTH + fcol] =
                        imgSrcExt[((row + frow) * imgWidthF + col + fcol) * 3 +
                                  1];

                    // b
                    bbuffer[frow * M_WIDTH + fcol] =
                        imgSrcExt[((row + frow) * imgWidthF + col + fcol) * 3 +
                                  2];
                }
            }

            std::nth_element(rbuffer,
                             rbuffer + M_WIDTH * M_HEIGHT / 2,
                             rbuffer + M_WIDTH * M_HEIGHT);
            std::nth_element(gbuffer,
                             gbuffer + M_WIDTH * M_HEIGHT / 2,
                             gbuffer + M_WIDTH * M_HEIGHT);
            std::nth_element(bbuffer,
                             bbuffer + M_WIDTH * M_HEIGHT / 2,
                             bbuffer + M_WIDTH * M_HEIGHT);

            imgDst[(row * imgWidth + col) * 3] =
                rbuffer[(M_HEIGHT * M_WIDTH) / 2];

            imgDst[(row * imgWidth + col) * 3 + 1] =
                gbuffer[(M_HEIGHT * M_WIDTH) / 2];

            imgDst[(row * imgWidth + col) * 3 + 2] =
                bbuffer[(M_HEIGHT * M_WIDTH) / 2];
        }
    }
}