...
int main(int argc, char *argv[]) {
  ...
  for (int32_t run = 0; run < number_of_runs_scalar; run++) {
    median2d_c(imgHeight, imgWidth, imgWidthF, imgSrcExt, imgRes);
  }
  ts_end = get_ts_ns();
  elapsed = ts_end - ts_start;

  perf = 1000.0 * double(imgHeight * imgWidth) * double(number_of_runs_scalar) /
         elapsed;
  ...
  perf = 1000.0 * double(imgHeight * imgWidth) * double(number_of_runs_vector) /
         elapsed;
